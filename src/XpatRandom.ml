...
(*-Creation de la seconde composante de la liste-*)
let seconde first=
   let rec seconde' l cpt first last=
      if cpt = 55 then List.rev l
      else  if (List.length l) =0 then seconde' (1::[first]) (cpt+2) first 1 (*-cas initial-*)
            else 
            if first>=last then seconde' ((first-last)::l) (cpt+1) last (first - last) 
            else seconde' ((first-last+randmax)::l) (cpt+1) last (first-last+randmax)
            in 
   seconde' [] 0 first 0  (*-Initialisation-*)
(*-Creation de la deuxieme composante de la liste-*)
   let first =
   let rec first' l last cpt=
      if cpt = 55 then List.rev l 
      else  if (List.length l) =0 then first' [0] 0 1 (*-Initialisation-*)
            else 
            let dernier = ((last+21) mod 55) in
   first' (dernier::l) dernier (cpt+1) 
   in
   first'[] 0 0 (*-Initialisation-*)
(*-Creation d'une file qui contient la 2 eme composant-*)
let fifolast couple = 
   let rec fifolast' couple l =
      match couple with
      | (first,last)::l' -> fifolast' l' (last::l)
      | _ -> List.rev l
   in 
   fifolast' couple [] (*-Initialisation de la liste a vide-*)
   (*-Split: diviser la liste en 2 selon n-*)
let splitt l cpt=
   let rec splitt' l l1  n =
      match l with
      |x::l' when n>0 -> splitt' (l') (x::l1) (n-1) (*-Remplir la premiere liste -*)
      |l'-> (List.rev (l1)), l'(*-Revnoi le reste-*)
   in
   splitt' l [] cpt
   (*-Effectuer le tirage de 2 FIFOs n fois-*)
let tirage fifo1 fifo2 n =
   let rec tirageaux fifo1 fifo2 n d =
      if n = 0 then fifo1,fifo2,d
      else
      let (n1,fifo1) = Fifo.pop fifo1 in
      let (n2,fifo2) = Fifo.pop fifo2 in
      let d =
         if n1>=n2 then n1-n2 else n1-n2+randmax
      in
   tirageaux (Fifo.push n2 fifo1) (Fifo.push d fifo2) (n - 1) d (*-Construction des mélange des listes-*)
   in
   tirageaux fifo1 fifo2 n 0
   (*-Reduire la valeur des nombres a max et creation de la liste finale-*)
let shortlist f1 f2 successifs =
   let rec shortlist' f1 f2 successifs res =
      match successifs with
      |[]-> res
      |_->
         let f1,f2,d = tirage f1 f2 1 in
         let d' = reduce d (List.length successifs) in
         let permutation = List.nth successifs d' in
         let successifs' = List.filteri (fun i _ -> i<>d') successifs in
         shortlist' f1 f2 successifs' (permutation::res)
   in
   shortlist' f1 f2 successifs []
(*-Creation de la liste des couple // consiste a combiner les é listes des composantes-*)
let comb n=
   List.combine (first) (seconde n)

(*-Traitement des permutations-*)
let shuffle graine =
  let sortedList = (*-Liste des couple trée-*)
      List.sort (fun (first,_) (first',_) -> compare first first') ((List.combine (first) (seconde graine))) in 
      let firstsub,secsub =
      splitt sortedList  24 in (*-division on 24-31-*)
   let cfirstsub,csecsub =
      fifolast firstsub , fifolast secsub in (*-creation des Files-*)
   let f1_init = Fifo.of_list csecsub in 
   let f2_init = Fifo.of_list cfirstsub in
   let perm=165 in (*-Faire 165 tirage-*)
   let f1max,f2max,_=tirage f1_init f2_init perm
   in 
   let permutation_graine_graine = (*-Construire la liste finale on fonction de graine-*)
    shortlist f1max f2max (List.init 52 (fun x -> x))
  in
  permutation_graine_graine
